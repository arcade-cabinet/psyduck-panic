...
export class GameLogic {
    ...
    addPanic(amt) {
        if (this.pu.shield > 0 || this.panicInvuln > 0)
            return;
    ...
    update(dt, now) {
        ...
        // Update timers
        if (this.panicInvuln > 0)
            this.panicInvuln--;
        if (this.nukeCd > 0)
            this.nukeCd -= 16.67 * dt;
        if (this.abilityCd.reality > 0)
            this.abilityCd.reality -= 16.67 * dt;
        if (this.abilityCd.history > 0)
            this.abilityCd.history -= 16.67 * dt;
        if (this.abilityCd.logic > 0)
            this.abilityCd.logic -= 16.67 * dt;
        if (this.pu.slow > 0)
            this.pu.slow -= 16.67 * dt;
        if (this.pu.shield > 0)
            this.pu.shield -= 16.67 * dt;
        if (this.pu.double > 0)
            this.pu.double -= 16.67 * dt;
        // Update enemies
        if (!this.bossPhase) {
            const cfg = WAVES[Math.min(this.wave, WAVES.length - 1)];
